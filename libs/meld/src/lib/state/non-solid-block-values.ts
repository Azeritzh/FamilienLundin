export class NonSolidBlockValues<TKey> {
	...
	addValuesFromOther(otherValues: NonSolidBlockValues<TKey>) {
	...
	addValuesFrom(key: TKey, values: GroupedNonSolidBlockValues) {
		if (values.hardness !== undefined)
			this.hardnessValues.set(key, values.hardness)
	}
}

export class GroupedNonSolidBlockValues {
	constructor(
